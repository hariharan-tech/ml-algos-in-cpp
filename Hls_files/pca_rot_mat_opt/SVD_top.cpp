...
//      rot_init<DIM>(row,col,a[test_sample],rot_matrix,rot_matrix_t);
//      matrix_mul<DIM>(rot_matrix,a,temp);
//      matrix_mul<DIM>(temp,rot_matrix_t,a);

      opt_rot_init<DIM>(row,col,a,rot_matrix) ;
      mult_A_Jt<DIM>(rot_matrix[row*DIM+col],rot_matrix[row*DIM+row],a,row,col,temp);
      mult_J_AJt<DIM>(rot_matrix[row*DIM+col] ,rot_matrix[row*DIM+row],temp,row,col,a);
